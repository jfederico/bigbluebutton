...
transcript_props = YAML::load(File.open('transcript.yml'))
...
begin
  ...
  if (playback == "transcript")
    ...
    logger = Logger.new("#{log_dir}/transcript/publish-#{meeting_id}.log", 'daily' )
    BigBlueButton.logger = logger

    recording_dir = bbb_props['recording_dir']
    raw_archive_dir = "#{recording_dir}/raw/#{meeting_id}"
    process_dir = "#{recording_dir}/process/transcript/#{meeting_id}"
    publish_dir = transcript_props['publish_dir']
    playback_protocol = bbb_props['playback_protocol']
    playback_host = bbb_props['playback_host']
    target_dir = "#{recording_dir}/publish/transcript/#{meeting_id}"
    ...
    if not FileTest.directory?(target_dir)
      ...
      BigBlueButton.logger.info("Finished publishing script transcript.rb successfully.")
      ...
      publish_done = File.new("#{recording_dir}/status/published/#{meeting_id}-transcript.done", "w")
  ...
  publish_done = File.new("#{recording_dir}/status/published/#{meeting_id}-transcript.fail", "w")
